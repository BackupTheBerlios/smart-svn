 ...
 * patTemplate
 *
 * $Id: patTemplate.php,v 1.73 2004/06/05 16:36:37 schst Exp $
 *
...
class patTemplate
{
    ...
    */
    function applyOutputFilter( $filter, $params = array() )
    {
        if( !is_object( $filter ) )
        {
            $filter = &$this->loadModule( 'OutputFilter', $filter );
        }
    ...
    */
    function applyInputFilter( $filter, $params = array() )
    {
        if( !is_object( $filter ) )
        {
            $filter = &$this->loadModule( 'InputFilter', $filter );
        }
        if( patErrorManager::isError( $filter ) )
            return $filter;
        ...
        foreach( $templates as $name => $spec )
        {
            ...
            if( isset( $this->_templates[$name]['attributes']['src'] ) && $this->_templates[$name]['attributes']['autoload'] == 'on' )
            {
                if( $this->_templates[$name]['loaded'] !== true )
                {
                    ...
                    {
                        $this->loadTemplateFromInput( $this->_templates[$name]['attributes']['src'], $this->_templates[$name]['attributes']['reader'], $name );
                    }
    ...
    * @return   boolean true, if the template could be parsed, false otherwise
    */
    function loadTemplateFromInput( $input, $reader = 'File', $parseInto = false )
    {
        $reader = &$this->loadModule( 'Reader', $reader );
        if( patErrorManager::isError( $reader ) )
        {
    ...
    */
    function parseTemplate( $template, $mode = 'w' )
    {
        ...
        {
            $this->_templates[$template]['result']      = '';
            $this->_templates[$template]['iteration']   =   0;
        }
        ...
        $loopCount = 0;
        for( $i = 0; $i < $this->_templates[$template]['loop']; $i++ )
        {
            $finished  = false;
            ...
            /**
             * add the globals to the variables
             */
            $this->_templates[$template]['vars'] = array_merge( $this->_globals, $this->_templates[$template]['vars'] );
    ...
    */
    function _parseVariables( $template )
    {
        ...
        foreach( $this->_templates[$template]['modifyVars'] as $varname => $modifier )
        {
            ...
            {
                $this->_templates[$template]['vars'][$varname] = call_user_func( $modifier['mod'], $this->_templates[$template]['vars'][$varname] );
                continue;
            }
            ...
            $this->_templates[$template]['vars'][$varname] = $mod->modify( $this->_templates[$template]['vars'][$varname], $modifier['params'] );
        }
    ...
    */
    function _fetchVariables( $template )
    {
        ...
        if( $this->_templates[$template]['attributes']['addsystemvars'] == 'on' )
        {
            $vars['PATTEMPLATE_VERSION'] = $this->_systemVars['appVersion'];

            $vars['PAT_LOOPS']      =   $this->_templates[$template]['loop'];
            $vars['PAT_IS_ODD']     =   ( $currentRow % 2 == 1 ) ? 'true' : 'false';
            $vars['PAT_IS_EVEN']    =   ( $currentRow % 2 == 0 ) ? 'true' : 'false';
            $vars['PAT_IS_FIRST']   =   ( $currentRow == 1 ) ? 'true' : 'false';
            $vars['PAT_IS_LAST']    =   ( $currentRow == $this->_templates[$template]['loop'] ) ? 'true' : 'false';
        }
        ...
        foreach( $this->_templates[$template]['copyVars'] as $dest => $src )
        {
             ...
             */
            $this->_fetchVariables( $src[0] );
            if( !isset( $this->_templates[$src[0]]['vars'][$src[1]] ) )
                    continue;
            $vars[$dest] = $this->_templates[$src[0]]['vars'][$src[1]];
        }
    ...
    * @see      addVar()
    */
    function parseIntoVar( $srcTmpl, $destTmpl, $var )
    {
        ...
        return $this->addVar( $destTmpl, $var, $this->_templates[$srcTmpl]['result'] );
    }
    ...
    */
    function clearTemplate( $name )
    {
        ...
        $this->_templates[$name]['iteration']   =   1;
