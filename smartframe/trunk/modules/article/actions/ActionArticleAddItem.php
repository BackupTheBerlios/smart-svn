...
/**
 * ActionArticleAddItem class 
 *
 *
 * $model->action('article','addItem',
 *                array('error'       => & array(),
...
class ActionArticleAddItem extends ActionArticleFileUploadBase
{
    ...
    function validate( $data = FALSE )
    {
        ...
        {
            throw new SmartModelException("'id_article' isnt numeric");
        }  
    ...
    private function addPicture( &$data, &$media_folder, &$file_info )
    {
        ...
        $this->model->action('common','imageThumb',
                             array('error'         => & $data['error'],
                                   ...
                                   'imgDestWidth'  => (int)$this->config['article']['thumb_width'],
     ...
     */    
    function getNewLastRank( $id_article, $table )
    {
        $sql = "
            SELECT
                rank
