...
/**
 * ActionArticleMoveItemRank class 
 *
 * move rank of article pictures or files
 *
 * USAGE:
 *
 * $model->action('article','moveItemRank',
 *                array('id_pic'  => int,     // one of both
 *                      'id_file' => int,     //
 *                      'dir'     => string)) // 'up' or 'down'
 *
 */
class ActionArticleMoveItemRank extends SmartAction
{                          
    /**
     * 
     *
    ...
    function validate( $data = FALSE )
    {
        ...
        {        
            throw new SmartModelException ('"id_article" must be defined'); 
        } 
        elseif(!is_int($data['id_article'])  )
        {        
            throw new SmartModelException ('"id_article" must be an integer'); 
        } 
