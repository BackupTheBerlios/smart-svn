...
class action_earchive_search extends action
{
    ...
    function perform( $data )
    {
        ...
        if( $data['get_list'] == TRUE )
        {
            ...
            $_l_table = "{$this->B->sys['db']['table_prefix']}earchive_lists AS l,";
        ...
        if((strtolower($data['bool']) == 'or'))
        {
            ...
            $sql = "
                SELECT DISTINCT 
                    {$_fields}
                FROM
                    {$this->B->sys['db']['table_prefix']}earchive_messages AS m,
                    {$this->B->sys['db']['table_prefix']}earchive_lists AS l,
                    {$this->B->sys['db']['table_prefix']}earchive_words_crc32 AS w
                WHERE
                    m.mid=w.mid 
                AND
                    m.lid=l.lid
