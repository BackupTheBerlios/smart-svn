 ...
 * @version $Id: DB.php,v 1.56 2004/06/19 17:51:35 arnaud Exp $
 * @package LiveUser
 * @category authentication
 */
class LiveUser_Admin_Auth_Container_DB extends LiveUser_Admin_Auth_Common
{
        ...
        $owner_user_id = null, $owner_group_id = null, $customFields = array())
    {
        ...
        if (isset($active)) {
            $active == true ? $active = 'Y' : $active = 'N';
            $updateValues[] =
                $this->authTableCols['optional']['is_active']['name'] . ' = ' . $this->dbc->quoteSmart($active);
        }

        if (isset($owner_user_id)) {
            $updateValues[] =
                $this->authTableCols['optional']['owner_user_id']['name'] . ' = ' . (int)$owner_user_id;
        }

        if (isset($owner_group_id)) {
            $updateValues[] =
                $this->authTableCols['optional']['owner_group_id']['name'] . ' = ' . (int)$owner_group_id;
        }
